...
public class ResultDeductible implements ResultFieldInterface {
	
	@Override
	public void setResultSource(SourceFieldInterface source) {
		throw new IllegalArgumentException("No result source required.");
	}

	@Override
	public boolean isOkToExecute(Transaction transaction) {
		if (transaction.isUserChangedDeductible()) return false;
		if (transaction.isDeductible()) return false;
		return true;
	}
