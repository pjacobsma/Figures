...
public class FilterSequenceAutoCommit<S, T> extends TextFieldTableCell<S, T> {

    protected TextField txtFldRef;
    protected boolean isEdit;
    
    public FilterSequenceAutoCommit() {
        this(null);
    }

    public FilterSequenceAutoCommit(final StringConverter<T> conv) {
        super(conv);
    	this.setTooltip(new Tooltip("Change this to change the order in which filters are executed"));
    }
    ...
    public static <S, T> Callback<TableColumn<S, T>, TableCell<S, T>> forTableColumn(final StringConverter<T> conv) {
        return list -> new FilterSequenceAutoCommit<S, T>(conv);
    }
